...
lmer7 <- lmer(value ~ treatment*gender+scale(ADS)+scale(significant.others)+scale(family)+scale(friends)+(1|subject),
              f_m_aversivesound_questionnaires[f_m_aversivesound_questionnaires$socialconcernlevel1=='low social concern',])
summary(lmer7)
Anova(lmer7,type = 3)
vif(lmer7)
r.squaredGLMM(lmer7)
...
effsize::cohen.d(value~treatment, f_m_aversivesound_questionnaires[f_m_aversivesound_questionnaires$gender=='female' & 
                                                                f_m_aversivesound_questionnaires$socialconcernlevel1=='low social concern',]  )
effsize::cohen.d(value~treatment, f_m_aversivesound_questionnaires[f_m_aversivesound_questionnaires$gender=='male' &
...
lmer8 <- lmer(value ~ treatment*gender+scale(ADS)+scale(significant.others)+scale(family)+scale(friends)+(1|subject),
               f_m_aversivesound_questionnaires[f_m_aversivesound_questionnaires$socialconcernlevel1=='high social concern',])
summary(lmer8)
Anova(lmer8,type = 3)
r.squaredGLMM(lmer8)
vif(lmer8)
...
nullmer8 <- lmer(value ~ (1|subject),
                  f_m_aversivesound_questionnaires[f_m_aversivesound_questionnaires$socialconcernlevel1=='high social concern',])
summary(nullmer8)
anova(lmer11,nullmer8)

# test whether there is a difference between high and low social concern in females in alone treatment group
lmer9 <- lmer(value ~ socialconcernlevel1+scale(ADS)+scale(significant.others)+scale(family)+scale(friends)+(1|subject),
...
f_m_aversivesound_questionnaires$socialconcernlevel1 = factor(f_m_aversivesound_questionnaires$socialconcernlevel1,
                                                              levels = c('low social concern','high social concern'))

p20 <- ggplot(f_m_aversivesound_questionnaires,aes(gender,value))
p20 + #geom_violin(aes(color = treatment),alpha=0.4,width=0.6,size=0.8) +
  geom_boxplot(aes(color = treatment), width = 0.10, position = position_dodge(0.6))+
  scale_color_manual(values = c("black", "grey65"))+
  facet_grid(~socialconcernlevel1)+
